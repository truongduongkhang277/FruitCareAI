...
def main():
    ...
    p.add_argument(
        "--output", default="models", help="Use a separate folder for a new experiment."
    )
    ...
    (out / "fruit_class_names.json").write_text(
    ...
    compile_model(1e-3)
    history = model.fit(
        train, validation_data=val, epochs=args.epochs_head, callbacks=[checkpoint]
    ).history
    if args.epochs_fine:
        base.trainable = True
        ...
        compile_model(1e-5)
        h2 = model.fit(
            train, validation_data=val, epochs=args.epochs_fine, callbacks=[checkpoint]
        ).history
        for key in history:
            history[key] += h2[key]
    ...
    for ax, metric in zip(axes, ["accuracy", "loss"]):
        ...
        ax.axvline(
            args.epochs_head - 0.5, linestyle="--", color="gray", label="Fine-tuning"
        )
